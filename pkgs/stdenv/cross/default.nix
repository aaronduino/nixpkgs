...
in lib.init bootStages ++ [
  ...
  (buildPackages: {
    ...
    overlays = overlays ++ crossOverlays
      ++ (if (crossSystem.isWasm || crossSystem.isRedox) then [(import ../../top-level/static.nix)] else []);
    selfBuild = false;
    stdenv = buildPackages.stdenv.override (old: rec {
      ...
      extraNativeBuildInputs = old.extraNativeBuildInputs
        ...
        ++ lib.optional
             (let f = p: !p.isx86 || p.libc == "musl" || p.libc == "wasilibc" || p.libc == "relibc" || p.isiOS; in f hostPlatform && !(f buildPlatform))
